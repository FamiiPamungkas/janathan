...
- Icons: **Phosphor Icons** webfont (npm package `@phosphor-icons/web`, MIT, **regular** weight). Use `<i class="ph ph-<name>" aria-hidden="true">` — icons inherit `color` so color via `text-*` classes (incl. `dark:`). Size via font-size classes (`text-xs`/`text-base`/`text-xl`/`text-2xl`), not `size-*`. Add `flex-none` only where needed (`.ph` already sets `flex-shrink: 0` via the `i.ph` rule in `index.css`). Fonts + CSS ship in `public/fonts/phosphor/` (gitignored, copied by `npm run build:icons` from `node_modules/@phosphor-icons/web/src/regular/` — part of `npm run build`); link it once in `layout.twig`. When adapting Pinemix components that carry Heroicons/Lucide-style inline SVGs, replace those with the equivalent Phosphor glyph too.
...
- **PHP syntax checks run on Windows, not WSL:** PHP executes via Laragon on Windows — do NOT first check for or try to install a Linux/WSL PHP (`which php`, `php` on PATH, etc.). To lint a file, invoke the Laragon PHP binary directly, e.g. `/mnt/c/laragon/bin/php/php-<version>-Win32-vs16-x64/php.exe -l path/to/file.php` (pick whichever version dir currently exists under `/mnt/c/laragon/bin/php/`). WebFetch the dev URL from `APP_URL` for runtime verification as usual.
