...
- Icons: **LineIcons** icon font (npm package `lineicons`, MIT). Use `<i class="lni lni-<name>" aria-hidden="true">` — icons inherit `color` so color via `text-*` classes (incl. `dark:`). Size via font-size classes (`text-xs`/`text-base`/`text-xl`/`text-2xl` == the old `size-3/4/5/6`), not `size-*`. Add `flex-none` only where needed (`.lni` already sets `flex-shrink: 0`). Icons ship in `public/fonts/lineicons/` (gitignored, copied by `npm run build:icons` from `node_modules/lineicons/dist/` — part of `npm run build`); link it once in `layout.twig`. When adapting Pinemix components that carry Heroicons/Lucide-style inline SVGs, replace those with the equivalent LineIcons glyph too.
