...
class HotspotController
{
    ...
    public function cookies(Request $request, Response $response): Response
    {
        ...
        try {
            $data = $this->hotspot->getCookies((int)$_SESSION['router_id']);
        } catch (\Throwable $e) {
            return $this->renderUnreachable($request, $response, $e, 'hotspot.cookies');
        }

        $html = $this->twig->render('pages/hotspot/cookies.twig', $data);
        $response->getBody()->write($html);

        return $response;
    }

    public function cookiesData(Request $request, Response $response): Response
    {
        if (($redirect = $this->withoutRouter($request, $response)) !== null) {
            return $redirect;
        }
    ...
    public function removeCookie(Request $request, Response $response, array $args): Response
    {
        if (($redirect = $this->withoutRouter($request, $response)) !== null) {
            return $redirect;
        }
        ...
        try {
            $this->hotspot->removeCookie((int)$_SESSION['router_id'], $args['id']);
        } catch (\Throwable $e) {
            $this->flash->add('error', $e->getMessage());
        ...
        $this->flash->add('success', $this->translator->trans('hotspot.cookies.flash.removed'));
